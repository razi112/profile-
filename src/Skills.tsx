...
export default function SkillsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white font-sans relative overflow-x-hidden">
        ...
        <div className="flex items-center justify-between px-4 sm:px-6 py-4 md:px-12 lg:px-24">
          ...
          <div className="flex items-center space-x-3">
            <a href="https://wa.me/918129489071" target="_blank" rel="noopener noreferrer"
              className="hidden md:flex items-center space-x-2 bg-lime-400 hover:bg-lime-300 text-black font-bold px-5 py-2 rounded-full font-medium transition-colors text-sm">
          ...
          {mobileMenuOpen && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
              ...
              <div className="px-4 py-6 space-y-1">
                ...
                <div className="pt-4">
                  <a href="https://wa.me/918129489071" target="_blank" rel="noopener noreferrer"
                    className="flex items-center justify-center space-x-2 bg-lime-400 hover:bg-lime-300 text-black font-bold px-6 py-3 rounded-full font-medium text-sm transition-colors w-full">
      ...
      >
        {/* Ambient glow */}
        <div className="absolute left-1/2 -translate-x-1/2 top-0 w-[600px] h-[400px] rounded-full blur-[130px] pointer-events-none -z-10"
        ...
        <motion.div variants={fadeInUp} className="flex flex-col items-center text-center space-y-4 mb-6">
          ...
          <h2 className="text-4xl md:text-5xl font-bold">
            <span className="text-lime-400">Skills</span> <span className="text-white">&</span> <span className="text-lime-400">Expertise</span>
          </h2>
        ...
        <motion.div variants={fadeInUp} className="text-center mb-12">
          <h3 className="text-2xl font-bold text-white mb-2">
            <span className="text-lime-400">Technical</span> Expertise
          </h3>
        ...
        <div className="space-y-5 pb-8">
          {skillBlocks.map(({ num, icon: Icon, title, desc, color, skills, comingSoon }) => (
            <motion.div key={num} variants={fadeInUp}
              ...
              className={`card-shine relative bg-[#111111] border border-white/5 rounded-2xl p-6 md:p-8 group transition-all duration-500 overflow-hidden ${comingSoon ? 'opacity-60' : 'hover:border-lime-400/15 hover:shadow-[0_0_40px_rgba(163,230,53,0.07)]'}`}>

              {comingSoon && (
                <div className="absolute inset-0 z-20 flex items-center justify-center rounded-2xl bg-[#0a0a0a]/60 backdrop-blur-[2px]">
                  <div className="flex items-center space-x-2 bg-black/60 border border-lime-400/30 rounded-full px-5 py-2">
              ...
              <div className="absolute left-0 top-6 bottom-6 w-0.5 rounded-full scale-y-0 group-hover:scale-y-100 transition-transform duration-500 origin-center" style={{ background: 'linear-gradient(to bottom, transparent, rgba(163,230,53,0.6), transparent)' }} />
              <div className="absolute -top-10 -right-10 w-32 h-32 rounded-full blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" style={{ background: 'rgba(163,230,53,0.06)' }} />
                ...
                <div className="md:w-56 shrink-0 space-y-3">
                  <div className="flex items-center space-x-3">
                    ...
                    <motion.div whileHover={{ rotate: 8, scale: 1.1 }} transition={{ type: 'spring', stiffness: 300 }}
                      ...
                      <Icon size={20} style={{ color: '#a3e635' }} />
                    </motion.div>
                  </div>
                ...
                <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  {skills.map((skill, i) => (
                    <motion.div key={skill}
                      ...
                      className="flex items-center space-x-2.5 bg-[#0a0a0a] border border-white/5 rounded-xl px-4 py-2.5 hover:border-lime-400/30 hover:bg-lime-400/5 transition-all duration-300 cursor-default">
